...
# ==========================================
# ⚡ OPTIMIZATION: Keep this cached to avoid re-downloading data
@st.cache_data
...
# ⚡ OPTIMIZATION: Cache the cleaning process
@st.cache_data
def clean_data(df):
...
# ⚡ OPTIMIZATION: This is the biggest speed boost. 
# It stops Python from re-calculating RSI/MACD every time you move a slider.
@st.cache_data
...
# ==========================================
# 🔹 MATHEMATICAL SIMULATION (CACHED)
# ==========================================
# ⚡ OPTIMIZATION: Cache the math simulation so the graph updates instantly
@st.cache_data
...
# ==========================================
# 🔹 ADVANCED FEATURES (HEAVY COMPUTE)
# ==========================================
...
# ⚡ OPTIMIZATION: We cannot cache this easily because it prints directly. 
# But since it's only triggered by a button, it won't slow down the main UI.
def neural_network_prediction(df):
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df["Date"].iloc[-30:], y=df["Price"].iloc[-30:].values.flatten(), name="Actual"))
    fig.add_trace(go.Scatter(x=future_dates, y=pred_prices.flatten(), name="Forecast", line=dict(dash='dot', color='red')))
